...
from . import World
from ..entities.agents.Dynamics import BicycleDynamics, update_with_perfect_controller, \
                                       curvature2tireangle
...
try: # hacky way to exclude dependencies on non-camera sensors
    from ..entities.sensors import Lidar, EventCamera
...
class Display:
    ...
    def render(self, gt_curvature=None):
        ...
        for i, agent in enumerate(self._agents_with_sensors):
            ...
            for j, (obs_name, obs) in enumerate(agent.observations.items()):
                ...
                if obs_name in cameras.keys():
                    ...
                    if gt_curvature:
                        gt_noodle = curvature2noodle(gt_curvature,
                                                     sensor.camera_param,
                                                     mode='camera')
                        obs = cv2.polylines(obs, [gt_noodle], False,
                                            (0, 0, 255), 3)
