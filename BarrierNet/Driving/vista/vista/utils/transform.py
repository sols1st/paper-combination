...
def compute_relative_latlongyaw(latlongyaw: Vec, latlongyaw_ref: Vec) -> Vec:
    ...
    rel_rot[1] = latlongyaw[2] - latlongyaw_ref[2] # TODO: rot from transformation matrix incorrect
    rel_xyyaw = vec2latlongyaw(rel_trans, rel_rot)
    rel_xyyaw[2] = pi2pi(rel_xyyaw[2])
    return rel_xyyaw
...
def pi2pi(angle: float) -> float:
    ...
    """
    if angle > np.pi: # NOTE: don't set >= here otherwise will cause infinite recursion
        angle = pi2pi(angle - 2 * np.pi)
    if angle <= -np.pi:
        angle = pi2pi(angle + 2 * np.pi)
    return angle
