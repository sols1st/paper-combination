...
from ..entities.agents.Dynamics import BicycleDynamics, CurvilinearDynamics

Vec = Union[List, Tuple, np.ndarray]
Dynamics = Union[BicycleDynamics, CurvilinearDynamics]


def agent2poly(agent: Car,
               ref_dynamics: Optional[Dynamics] = None) -> Box:
    """ Convert Agent object to polygon w.r.t. a reference dynamics.

    Args:
        agent (Car): An agent with valid dynamics (pose and vehicle state).
        ref_dynamics (Dynamics):
