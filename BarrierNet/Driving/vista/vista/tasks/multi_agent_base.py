...
def default_terminal_condition(task, agent_id, **kwargs):
    ...
    def _check_crash():
        ...
        return crashed, overlap

    def _check_passed():
        other_agents = [_a for _a in task.world.agents if _a.id != agent_id]
        is_front = [agent.frame_index > _a.frame_index for _a in other_agents]
        dist = [np.linalg.norm(agent.ego_dynamics.numpy()[:2] - _a.ego_dynamics.numpy()[:2]) for _a in other_agents]
        n_passed = np.sum([v1 and (v2 > 10.) for v1, v2 in zip(is_front, dist)])
        return n_passed

    out_of_lane = _check_out_of_lane()
    exceed_max_rot = _check_exceed_max_rot()
    crashed, overlap = _check_crash()
    n_passed = _check_passed()
    done = out_of_lane or exceed_max_rot or crashed or agent.done or (n_passed > 0)
    other_info = {
        'done': done,
...
class MultiAgentBase:
    ...
    def step(self, actions, dt=1 / 30.):
        ...
        # Get info
        infos = infos_from_terminal_condition
    ...
    def _randomly_place_agent(self, agent: Car):
        ...
        def _close_to_any_agent(_pose, _dist_thresh=30):
            return np.any([
                ...
                _dist_thresh for _a in self.world.agents if _a.id != agent.id
            ])

        resample = True
        while resample:
            if self.config['init_dist_range'] is not None: # NOTE: check dist_thresh in _close_to_any_agent
                tgt_dist = self._rng.uniform(*self.config['init_dist_range'])

                dist_from_ego = np.linalg.norm(road[:, :2], axis=1)
                zero_crossings = np.where(np.diff(np.sign(tgt_dist - dist_from_ego)))
                tgt_idx = zero_crossings[0][0]
                tgt_pose = road[tgt_idx].copy()
                resample = _close_to_any_agent(tgt_pose, 5.)
            else:
